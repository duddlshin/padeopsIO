...
class PadeOpsViz:
    ...
    def __init__(
        ...
        print(
            "PadeOpsViz initialized using info files at time:"
    ...
    def ReadVelocities_budget(self, tidx, n, budget_number, terms):
        self.budget = np.zeros(
            (self.nx, self.ny, self.nz, np.size(terms))
        )  # fourth dim. used to be np.size(terms)+1. Changed 12/08/21
        ind = 0
        for i in terms:
            u_fname = (
                self.outputdir_name
            ...
            self.budget[:, :, :, ind] = self.u_normfactor * temp.reshape(
    ...
    def ReadVelocities(self, tidx, readAll=True):
        ...
        if readAll:
            ...
            self.u = self.u_normfactor * self.u.reshape(
                (self.nx, self.ny, self.nz), order="F"
            )
            v_fname = (
                self.outputdir_name
            ...
            self.v = self.u_normfactor * self.v.reshape(
                (self.nx, self.ny, self.nz), order="F"
            )
            w_fname = (
                self.outputdir_name
            ...
            self.w = self.u_normfactor * self.w.reshape(
                (self.nx, self.ny, self.nz), order="F"
            )
            print(
                "PadeOpsViz loaded the velocity fields at time:"
                + "{:.06f}".format(self.time)
            )
        else:
            u_fname = (
            ...
            print(
                "PadeOpsViz loaded the u velocity field at time:"
                + "{:.06f}".format(self.time)
            )
    ...
    def Read_BPF_Velocities(self, tidx):
        ...
        print(
            "PadeOpsViz loaded the BPF velocity fields at time:"
            + "{:.06f}".format(self.time)
        )
    ...
    def ReadPotTemp(self, tidx):
        ...
        print(
            "PadeOpsViz loaded the Potential Temperature field at time:"
            + "{:.06f}".format(self.time)
        )
    ...
    def ReadPressure(self, tidx):
        ...
        print(
            "PadeOpsViz loaded the Pressure field at time:"
            + "{:.06f}".format(self.time)
        )
    ...
    def plot_xy(
        ...
        if ax is None:  # modified 01/19/2022
            fig = plt.figure(
                num=None, figsize=fsize, dpi=pix, facecolor="w", edgecolor="k"
            )
    ...
    def plot_xy_slice(
        self, field, vlim, fsize=(4, 4), pix=200, colormap="jet", interp="none"
    ):
    ...
    def plot_spacetime_slice(
        self, field, arrx, xlab, fsize=(4, 4), pix=200, colormap="jet", interp="none"
    ):
    ...
    def plot_yz_u_and_P(self, xid=0, fsize=(8, 4), pix=200, cmap1="jet", cmap2="jet"):
        ...
        im1 = axarr[0].pcolormesh(
            self.yG[xid, :, :], self.zG[xid, :, :], self.u[xid, :, :], cmap=cmap1
        )
        ...
        im2 = axarr[1].pcolormesh(
            self.yG[xid, :, :], self.zG[xid, :, :], self.Press[xid, :, :], cmap=cmap1
        )
    ...
    def plot_xy_2field(
        ...
        im1 = axarr[0].pcolormesh(
            xG, yG, f1[:, :, zid], cmap=cmap1
        )  # ,vmin=clim1[0], vmax=clim1[1])
        axarr[0].set_xlim([0, self.Ly])
        axarr[0].set_ylim([0, self.Lz])
        axarr[0].set_ylabel("Z")
        axarr[0].set_xlabel("Y")
    ...
    def get_xSpectra_from_xy_plane(self, f, zid):
        k1 = np.fft.fftshift(
            np.arange(-self.nx / 2, self.nx / 2) * (2 * np.pi / (self.nx * self.dx))
        )
    ...
    def TwoPtCorr_xy(self, f, g):
        ...
        rx, ry = np.meshgrid(
            np.linspace(-self.Lx, self.Lx, 2 * self.nx - 1),
            np.linspace(-self.Ly, self.Ly, 2 * self.ny - 1),
            indexing="ij",
        )
        return corr2d, rx, ry
    ...
    def TwoPtCorr_xt(self, flarge, glarge, steps=1):
        corrsave = np.zeros(
            (2 * self.nx - 1, 2 * self.times.size - 1), dtype=np.float64, order="F"
        )
        idx = 0
        ...
        rx, rt = np.meshgrid(
            np.linspace(-self.Lx, self.Lx, 2 * self.nx - 1),
            np.linspace(-self.times[-1], self.times[-1], 2 * self.times.size - 1),
            indexing="ij",
        )
    ...
    def TwoPtCorr_yt(self, flarge, glarge, steps=1):
        corrsave = np.zeros(
            (2 * self.ny - 1, 2 * self.times.size - 1), dtype=np.float64, order="F"
        )
        idx = 0
