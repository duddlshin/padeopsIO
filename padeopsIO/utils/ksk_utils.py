"""
Temporary extra budget functions for DeficitIO
#TODO: Merge with math_utils.py
...
def tke_calc(io):
    ...
    if isinstance(io, padeopsIO.DeficitIO):
        ...
        io.budget["TKE"] = (
            0.5
            * (io.budget["delta_uu"] + io.budget["delta_vv"] + io.budget["delta_ww"])
            + io.budget["delta_u_base_u"]
            + io.budget["delta_v_base_v"]
            + io.budget["delta_w_base_w"]
        )
...
def mke_calc(io):
    ...
    if isinstance(io, padeopsIO.DeficitIO):
        ...
        io.budget["MKE"] = 0.5 * (
            io.budget["ubar"] ** 2 + io.budget["vbar"] ** 2 + io.budget["wbar"] ** 2
        )
...
def flux_calc(
    ...
    if not coords:
        ...
        xid, yid, zid = io.get_xids(
            x=coords[0], y=coords[1], z=coords[2], return_none=True, return_slice=True
        )
    ...
    if yc is not None:
        ...
        if len(io.xLine[xid]) != len(yc) and len(io.xLine) != len(yc):
            print(
                "yc must have the same length as the x coordinates or be the total length of the domain"
            )
            return
        else:
            ...
            for term in budget_terms:
                ...
                for i in range(xid.start, xid.stop):
                    ...
                    tmp.append(
                        np.sum(
                            io.budget[vel][xid, yid, zid]
                            * io.budget[term][xid, yid, zid],
                            axis=axis,
                        )
                        * dx1
                        * dx2
                ...
                if isinstance(axis, tuple) and 0 in axis:
                    io.budget[term + "_flux_" + direction + suffix] = np.sum(
                        io.budget[term + "_flux_" + direction + suffix], axis=0
                    )
    else:
        for term in budget_terms:
            ...
            if term + "_flux_" + direction + suffix in io.budget and not overwrite:
                ...
                if term not in io.budget:
                    io.read_budgets(budget_terms=[term])
                io.budget[term + "_flux_" + direction + suffix] = (
                    np.sum(
                        io.budget[vel][xid, yid, zid] * io.budget[term][xid, yid, zid],
                        axis=axis,
                    )
                    * dx1
                    * dx2
...
def TI_calc_hh(prim, pre):
    ...
    _, yid, zid = prim.get_xids(
        y=[-1, 1], z=[0, 1], return_none=True, return_slice=True
    )
