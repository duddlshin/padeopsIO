...
class Grid3:
    ...
    def get_xids(
        self, xlim=None, ylim=None, zlim=None, return_slice=False, return_none=None
    ):
        """Calls get_xids()"""
        xi_ids = get_xids(
            x=self.x,
    ...
    def to_meshgrid(self):
        return np.meshgrid(
            *self.get_axes(named=False, special_if_1d=False), indexing="ij"
        )
    ...
    def _adjust_grid(self, keys):
        ...
        if len(keys) == 1 and self.grid.ndim > 1:
            keys = (
                np.asarray(keys[0]),
                Ellipsis,
            )  # append an ellipsis on the end, consistent with numpy?
