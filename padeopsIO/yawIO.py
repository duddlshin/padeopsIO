...
class YawIO(BudgetIO):
    ...
    def __init__(self, dir_name, **kwargs):
        ...
        if self.associate_nml:
            try:
                ...
                self.uInflow = self.input_nml["ad_coriolisinput"]["uinflow"] * np.cos(
                    self.yaw * np.pi / 180.0
                )
                self.vInflow = self.input_nml["ad_coriolisinput"]["uinflow"] * -np.sin(
                    self.yaw * np.pi / 180.0
                )
                ...
                if self.verbose:
                    print(repr(e))
                    print(
                        "Initializing YawIO, could not find `ad_coriolisinput` in the Namelist input."
                    )
                warnings.warn(
                    "Could not find `ad_coriolisinput` in the Namelist input."
                )
    ...
    def read_turb_vel(
        self, tidx=None, turb=1, steady=True, u=True, v=True, rotate=False
    ):
        """
        ...
        for i, ui in enumerate((u_read, v_read)):
            if ui:
                ...
                fname = self.dir_name + "/Run{:02d}_t{:06d}_turb{:s}{:02}.vel".format(
                    self.runid, tidx, u_string, turb
                )
        ...
        if rotate:
            ...
            ret = (
                new_ret  # there are an infinite number of better ways to have done this
            )
    ...
    def rotate_uv(self, overwrite=True, load_fields=False, u=None, v=None):
        ...
        if u is None and v is None:
            if load_fields:
                self.read_fields(
                    field_terms=["u", "v"]
                )  # TODO: there is no overwrite check on this!
                u = self.field["u"]
                v = self.field["v"]
            else:
